...
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = (await request.json()) as {
      ...
      allowPause?: unknown;
      allowResume?: unknown;
      allowBudgetChanges?: unknown;
      maxBudgetIncreasePct?: unknown;
      maxDailyBudgetUsd?: unknown;
    ...
    if (body.executionEnabled === true && body.dryRunOnly === false) {
      ...
      if (!grantedPermissions.includes("ads_management"))
        return NextResponse.json(
          { error: "El token no tiene ads_management." },
          { status: 409 },
        );
    }
    const update = {
      ...
      allow_budget_changes: body.allowBudgetChanges === true,
      max_budget_increase_pct: Math.min(
        100,
        Math.max(0, Number(body.maxBudgetIncreasePct) || 20),
      ),
      max_daily_budget_usd: Math.max(1, Number(body.maxDailyBudgetUsd) || 100),
      cooldown_minutes: Math.min(
        1440,
        Math.max(0, Number(body.cooldownMinutes) || 30),
      ),
      updated_by: user.id,
