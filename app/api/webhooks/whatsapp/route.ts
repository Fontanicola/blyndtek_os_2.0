...
type Message = { id?: string; from?: string; timestamp?: string; type?: string; text?: { body?: string }; referral?: Record<string, unknown> };
type Status = { id?: string; status?: string };
type Payload = { entry?: Array<{ changes?: Array<{ value?: { metadata?: { phone_number_id?: string }; contacts?: Array<{ wa_id?: string; profile?: { name?: string } }>; messages?: Message[]; statuses?: Status[] } }> }> };
...
function eventTimestamp(value?: string) {
  const seconds = Number(value || 0); return seconds ? new Date(seconds * 1000).toISOString() : new Date().toISOString();
}
...
export async function GET(request: NextRequest) {
  ...
  if (mode === "subscribe" && token && token === process.env.WHATSAPP_WEBHOOK_VERIFY_TOKEN?.trim()) return new NextResponse(challenge || "", { status: 200 });
  return NextResponse.json({ error: "Verificación inválida." }, { status: 403 });
}

export async function POST(request: NextRequest) {
  const raw = await request.text();
  if (!validSignature(raw, request.headers.get("x-hub-signature-256"))) return NextResponse.json({ error: "Firma inválida." }, { status: 401 });
  let payload: Payload;
  try { payload = JSON.parse(raw) as Payload; } catch { return NextResponse.json({ error: "JSON inválido." }, { status: 400 }); }
  const db = createUntypedAdminClient();

  for (const entry of payload.entry ?? []) for (const change of entry.changes ?? []) {
    const value = change.value; if (!value) continue;
    for (const status of value.statuses ?? []) if (status.id) await db.from("whatsapp_messages").update({ status: status.status || null }).eq("id", status.id);
