...
import { Badge, Button, Card, DataTable, DataTableBody, DataTableCell, DataTableHead, DataTableHeader, DataTableRow, EmptyState, FilterPopover } from "@/components/ui";
...
function shortDate(value: string | number) { return new Date(String(value)).toLocaleDateString("es-AR", { day: "2-digit", month: "short" }); }
function funnelStageKey(stage: string) {
  if (stage === "ganado") return "won";
  if (stage === "cotizacion") return "proposal";
  if (["diagnostico_ofrecido", "diagnostico_pagado"].includes(stage)) return "diagnosis";
...
function MetricCard({ label, value, detail, tone = "default" }: { label: string; value: string; detail: string; tone?: "default" | "success" | "warning" }) {
  return <Card padding="sm" className={cn("min-w-0 bg-gradient-to-br to-white", tone === "success" ? "border-success/20 from-success/10" : tone === "warning" ? "border-orange-500/20 from-orange-500/10" : "border-signal/15 from-signal/5")}>
    <p className="text-[11px] font-label uppercase tracking-[0.08em] text-graphite">{label}</p>
...
export default function MarketingPage() {
  ...
  return <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto p-4 sm:p-5">
    <div className="flex flex-wrap items-center justify-between gap-2">
      <div className="flex max-w-full items-center gap-1 overflow-x-auto rounded-card border border-line-soft bg-white p-1 shadow-sm">
        {tabs.map((item) => <button key={item.value} type="button" onClick={() => setTab(item.value)} className={cn("shrink-0 rounded-md px-3 py-2 text-sm font-label transition-colors", tab === item.value ? "bg-signal text-white shadow-sm" : "text-graphite hover:bg-paper hover:text-carbon")}>{item.label}</button>)}
      </div>
      <div className="flex items-center gap-2">
        <FilterPopover align="right" label={periods.find((item) => item.value === period)?.label || "Período"} activeCount={period === "30d" ? 0 : 1}>
          <p className="mb-3 text-xs font-label uppercase tracking-wide text-graphite">Período de análisis</p><div className="grid grid-cols-2 gap-2">{periods.map((item) => <button key={item.value} type="button" onClick={() => setPeriod(item.value)} className={cn("rounded-md border px-3 py-2 text-sm", period === item.value ? "border-signal bg-signal-light font-label text-signal" : "border-line-soft text-graphite hover:bg-paper")}>{item.label}</button>)}</div><p className="mt-3 text-xs leading-5 text-graphite">Este filtro actualiza métricas, gráficos, campañas y atribución.</p>
        </FilterPopover>
        {connection ? <span title={`Meta: ${friendlyStatus(connection.status)}`} className={cn("h-2.5 w-2.5 rounded-full", connection.status === "connected" ? "bg-success" : "bg-warning")} /> : null}
        {canSync ? <Button variant="secondary" size="sm" className="h-9 w-9 px-0" aria-label="Sincronizar todos los canales" onClick={() => void synchronize()} disabled={syncing || missingConfig}><RefreshIcon className={cn(syncing && "animate-spin")} size={16} /></Button> : null}
      </div>
    </div>
    ...
    {notice ? <div className="flex items-start gap-3 rounded-md border border-line-soft bg-paper px-4 py-3 text-sm text-graphite"><CheckCircleIcon className="mt-0.5 shrink-0 text-success" size={17} /><span>{notice}</span></div> : null}
    ...
    {overview && tab === "resumen" ? <>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card padding="none" className="min-w-0 overflow-hidden"><div className="flex items-start justify-between border-b border-line-soft px-5 py-4"><div><p className="font-title text-lg text-carbon">Inversión diaria</p><p className="mt-1 text-xs text-graphite">Ritmo de gasto en el período.</p></div><BarChartIcon className="text-signal" size={20}/></div>{overview.trend.length ? <div className="h-[270px] px-2 pb-3 pt-5"><ResponsiveContainer width="100%" height="100%"><AreaChart data={overview.trend} margin={{ left: -8, right: 16 }}><defs><linearGradient id="spendArea" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={chartTheme.colors.signal} stopOpacity={0.3}/><stop offset="95%" stopColor={chartTheme.colors.signal} stopOpacity={0}/></linearGradient></defs><CartesianGrid stroke={chartTheme.grid.stroke} vertical={false}/><XAxis dataKey="date" tick={chartTheme.axis.tick} axisLine={false} tickLine={false} tickFormatter={shortDate}/><YAxis tick={chartTheme.axis.tick} axisLine={false} tickLine={false} tickFormatter={(value) => `$${value}`}/><Tooltip contentStyle={{ border: `1px solid ${chartTheme.colors.line}`, borderRadius: 10, fontSize: 12 }} formatter={(value: number | string) => [decimalMoney.format(Number(value)), "Inversión"]}/><Area type="monotone" dataKey="spend" name="Inversión" stroke={chartTheme.colors.signal} strokeWidth={2.5} fill="url(#spendArea)"/></AreaChart></ResponsiveContainer></div> : <div className="px-6 py-12"><EmptyState icon={BarChartIcon} titulo="Sin inversión todavía" descripcion="El histórico aparecerá después de la entrega de campañas."/></div>}</Card>
        <Card padding="none" className="min-w-0 overflow-hidden"><div className="border-b border-line-soft px-5 py-4"><p className="font-title text-lg text-carbon">Demanda y atribución</p><p className="mt-1 text-xs text-graphite">Comparación diaria entre Meta y CRM.</p></div><div className="h-[270px] px-2 pb-3 pt-5"><ResponsiveContainer width="100%" height="100%"><LineChart data={overview.trend} margin={{ left: -24, right: 16 }}><CartesianGrid stroke={chartTheme.grid.stroke} vertical={false}/><XAxis dataKey="date" tick={chartTheme.axis.tick} axisLine={false} tickLine={false} tickFormatter={shortDate}/><YAxis tick={chartTheme.axis.tick} axisLine={false} tickLine={false} allowDecimals={false}/><Tooltip contentStyle={{ border: `1px solid ${chartTheme.colors.line}`, borderRadius: 10, fontSize: 12 }}/><Legend iconType="circle" iconSize={7} wrapperStyle={{ fontSize: 11 }}/><Line type="monotone" dataKey="platformLeads" name="Leads Meta" stroke="#7c3aed" strokeWidth={2.5} dot={false}/><Line type="monotone" dataKey="crmLeads" name="Leads CRM" stroke={chartTheme.colors.success} strokeWidth={2.5} dot={false}/></LineChart></ResponsiveContainer></div></Card>
      </div>
      <div className="grid gap-4 lg:grid-cols-[1.4fr_1fr]"><Card padding="none" className="overflow-hidden"><div className="border-b border-line-soft px-5 py-4"><p className="font-title text-lg text-carbon">Embudo atribuible</p><p className="mt-1 text-xs text-graphite">Conversión acumulada desde el lead.</p></div><div className="grid gap-3 p-5 sm:grid-cols-5">{overview.funnel.map((stage, index) => <div key={stage.key} className="rounded-card border border-line-soft bg-paper/50 p-4"><p className="text-xs font-label text-graphite">{stage.label}</p><p className="mt-2 font-title text-2xl text-carbon">{integer.format(stage.count)}</p><p className="mt-1 text-xs text-graphite">{index ? `${pct(stage.conversionFromLead)} del total` : "Entrada"}</p><div className="mt-3 h-1.5 overflow-hidden rounded-pill bg-white"><div className="h-full rounded-pill bg-signal" style={{ width: `${Math.max(3, (stage.conversionFromLead ?? 0) * 100)}%` }}/></div></div>)}</div></Card><Card><p className="font-title text-lg text-carbon">Lectura rápida</p><div className="mt-4 space-y-3">{[["CTR", metricPct(kpis?.ctr || 0)], ["CPL", kpis?.costPerLead ? decimalMoney.format(kpis.costPerLead) : "—"], ["CPQL", kpis?.costPerQualifiedLead ? decimalMoney.format(kpis.costPerQualifiedLead) : "—"], ["Cash ROAS", ratio(kpis?.cashRoas ?? null)]].map(([label, value]) => <div key={label} className="flex items-center justify-between border-b border-line-soft pb-3 last:border-0"><span className="text-sm text-graphite">{label}</span><span className="font-title text-lg text-carbon">{value}</span></div>)}</div></Card></div>
    </> : null}
    ...
    {overview && tab === "creatividad" ? overview.creatives.length ? <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><MetricCard label="Piezas activas" value={integer.format(overview.creatives.filter((item) => item.status === "ACTIVE").length)} detail={`${overview.creatives.length} creatividades sincronizadas`}/><MetricCard label="Mejor CTR" value={metricPct(Math.max(...overview.creatives.map((item) => item.ctr), 0))} detail="Respuesta al mensaje y al hook" tone="success"/><MetricCard label="Mejor CPL" value={overview.creatives.some((item) => item.cpl) ? decimalMoney.format(Math.min(...overview.creatives.flatMap((item) => item.cpl ? [item.cpl] : []))) : "—"} detail="Entre piezas con conversiones"/></div><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{overview.creatives.map((row) => <Card key={`${row.adId}-${row.id}`} padding="none" className="overflow-hidden"><div className="relative">{row.thumbnailUrl ? <img src={row.thumbnailUrl} alt={`Preview de ${row.adName}`} className="aspect-video w-full bg-paper object-cover"/> : <div className="flex aspect-video items-center justify-center bg-gradient-to-br from-signal-light to-paper text-signal"><MegaphoneIcon size={34}/></div>}<div className="absolute left-3 top-3"><Badge variant={statusVariant(row.status)}>{friendlyStatus(row.status)}</Badge></div></div><div className="p-4"><p className="line-clamp-1 font-label text-carbon">{row.adName}</p><p className="mt-1 line-clamp-2 min-h-10 text-xs leading-5 text-graphite">{row.title || row.body || row.creativeName}</p><div className="mt-4 grid grid-cols-3 gap-2 border-y border-line-soft py-3 text-center"><div><p className="font-title text-lg text-carbon">{metricPct(row.ctr)}</p><p className="text-[10px] text-graphite">CTR</p></div><div><p className="font-title text-lg text-carbon">{pct(row.hookRate)}</p><p className="text-[10px] text-graphite">Hook</p></div><div><p className="font-title text-lg text-carbon">{row.cpl ? decimalMoney.format(row.cpl) : "—"}</p><p className="text-[10px] text-graphite">CPL</p></div></div><div className="mt-3 flex items-center justify-between"><p className="text-xs text-graphite">{money.format(row.spend)} · {integer.format(row.platformLeads)} leads</p>{permissions.canCreateActions && row.status === "ACTIVE" ? <Button variant="ghost" size="sm" disabled={actionBusy === row.adId} onClick={() => void proposePause("ad", row.adId, row.adName)}>Gestionar</Button> : null}</div></div></Card>)}</div></div> : <EmptyModule title="Todavía no hay creatividades" description="Las piezas aparecerán con su inversión, respuesta y costo por lead después de sincronizar Meta." /> : null}

    {overview && tab === "embudo" ? <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Landing → Lead" value={pct(kpis?.landingPageViews ? (kpis.platformLeads / kpis.landingPageViews) : null)} detail={`${integer.format(kpis?.landingPageViews || 0)} visitas a landing`}/><MetricCard label="Meta → CRM" value={pct(kpis?.platformLeads ? (kpis.crmLeads / kpis.platformLeads) : null)} detail="Cobertura de atribución"/><MetricCard label="Lead → Calificado" value={pct(kpis?.crmLeads ? (kpis.qualifiedLeads / kpis.crmLeads) : null)} detail="Calidad de la demanda" tone="success"/><MetricCard label="Lead → Venta" value={pct(kpis?.crmLeads ? (kpis.wonLeads / kpis.crmLeads) : null)} detail={`${integer.format(kpis?.wonLeads || 0)} ganados`} tone="success"/></div><div className="overflow-x-auto pb-2"><div className="grid min-w-[1180px] grid-cols-5 gap-3">{overview.funnel.map((stage, index) => { const leadsInStage = overview.funnelLeads.filter((lead) => funnelStageKey(lead.stage) === stage.key); return <div key={stage.key} className="rounded-card border border-line-soft bg-paper/60 p-3"><div className="mb-3 flex items-center justify-between"><div><p className="font-label text-carbon">{stage.label}</p><p className="text-[11px] text-graphite">{index ? `${pct(stage.conversionFromPrevious)} conversión` : "Entrada"}</p></div><span className="flex h-8 min-w-8 items-center justify-center rounded-full bg-white px-2 font-title text-carbon">{stage.count}</span></div><div className="space-y-2">{leadsInStage.map((lead) => <div key={lead.id} className="rounded-md border border-line-soft bg-white p-3 shadow-sm"><p className="truncate text-sm font-label text-carbon">{lead.company}</p><p className="mt-0.5 truncate text-xs text-graphite">{lead.name}</p><p className="mt-2 truncate text-[10px] text-signal">{lead.campaign || "Sin campaña"}</p></div>)}{!leadsInStage.length ? <div className="rounded-md border border-dashed border-line p-4 text-center text-xs text-graphite">Sin leads en esta etapa</div> : null}</div></div>; })}</div></div></div> : null}
