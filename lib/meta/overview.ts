...
import type { MetaCampaignRow, MetaCreativeRow, MetaFunnelStage, MetaGuardrails, MetaOverview, MetaPeriod } from "@/types/meta";
...
const defaultGuardrails: MetaGuardrails = { targetCpl: 60, targetCpql: 180, targetCashRoas: 3, minLinkCtr: 0.8, maxFrequency: 3.5, maxAttributionGapPct: 25, minSpendForAlert: 100, staleSyncHours: 36 };

function emptyDeliveryMetrics() {
  return { spend: 0, impressions: 0, reach: 0, linkClicks: 0, landingPageViews: 0, platformLeads: 0, videoPlays3s: 0, videoPlays15s: 0 };
}
...
export async function getMetaOverview(period: MetaPeriod): Promise<MetaOverview> {
  ...
  const totals = insights.reduce((acc, row) => {
    ...
    acc.videoPlays3s += n(row.video_plays_3s); acc.videoPlays15s += n(row.video_plays_15s);
    return acc;
  ...
  for (const row of insights) {
    for (const [id, map] of [[row.campaign_id, insightsByCampaign], [row.ad_id, insightsByAd]] as const) {
      ...
      current.videoPlays3s += n(row.video_plays_3s); current.videoPlays15s += n(row.video_plays_15s);
      map.set(String(id), current);
    }
  }

  const campaigns: MetaCampaignRow[] = campaignsData.map((campaign) => {
    const metrics = insightsByCampaign.get(campaign.id) ?? emptyDeliveryMetrics();
  ...
  const creatives: MetaCreativeRow[] = adsData.map((ad) => {
    const creative = creativeById.get(ad.creative_id as string); const metrics = insightsByAd.get(ad.id) ?? emptyDeliveryMetrics();
    return { id: (creative?.id as string) || ad.id, adId: ad.id, adName: ad.name, creativeName: (creative?.name as string) || ad.name,
      ...
      spend: metrics.spend, impressions: metrics.impressions, linkClicks: metrics.linkClicks, platformLeads: metrics.platformLeads,
      ctr: metrics.impressions ? metrics.linkClicks / metrics.impressions * 100 : 0, cpl: divide(metrics.spend, metrics.platformLeads),
      videoPlays3s: metrics.videoPlays3s, videoPlays15s: metrics.videoPlays15s,
      hookRate: divide(metrics.videoPlays3s, metrics.impressions), holdRate: divide(metrics.videoPlays15s, metrics.videoPlays3s) };
  }).sort((a, b) => b.spend - a.spend);
  ...
  const storedRecommendations = metaTablesUnavailable ? [] : (recommendationsResult.data ?? []).map((row) => ({ id: row.id, severity: row.severity, status: row.status, ruleKey: row.rule_key, entityType: row.entity_type, entityId: row.entity_id, title: row.title, rationale: row.rationale, recommendedAction: row.recommended_action, detectedAt: row.detected_at, lastDetectedAt: row.last_detected_at || row.detected_at, occurrences: n(row.occurrences) || 1 }));
  const detectedAt = new Date().toISOString();
  const automaticRecommendations = [] as MetaOverview["recommendations"];
  if (!config.configured) automaticRecommendations.push({ id: "configuration", severity: "critical", status: "open", ruleKey: "configuration", entityType: "account", entityId: null, title: "Completar la conexión con Meta", rationale: "Sin credenciales server-side no es posible leer inversión ni entrega publicitaria.", recommendedAction: "Cargar las variables pendientes en Vercel y ejecutar una sincronización manual.", detectedAt, lastDetectedAt: detectedAt, occurrences: 1 });
