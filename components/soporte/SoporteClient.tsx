...
function TicketsList({ items, onUpdate, onCreate }: { items: SoporteTicket[]; onUpdate: (endpoint: string, payload: Record<string, unknown>) => void; onCreate: () => void }) {
  ...
  return <div className="space-y-3">{items.map((item) => <Card key={item.id} className="p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h2 className="font-label text-carbon">{item.titulo}</h2><Badge variant={statusVariant(item.prioridad)}>{item.prioridad}</Badge><Badge>{item.cliente_nombre}</Badge></div><p className="mt-2 text-sm leading-6 text-graphite">{item.descripcion}</p></div><select value={item.estado} onChange={(event) => onUpdate(`/api/soporte/tickets/${item.id}`, { estado: event.target.value })} className="rounded-component border border-line bg-white px-3 py-2 text-sm"><option value="abierto">Abierto</option><option value="en_progreso">En progreso</option><option value="esperando_cliente">Esperando cliente</option><option value="resuelto">Resuelto</option><option value="cerrado">Cerrado</option></select></div></Card>)}</div>;
}
...
function UpsellList({ items, onUpdate, onCreate }: { items: OportunidadUpsell[]; onUpdate: (endpoint: string, payload: Record<string, unknown>) => void; onCreate: () => void }) {
  ...
  return <div className="space-y-3">{items.map((item) => <Card key={item.id} className="p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h2 className="font-label text-carbon">{item.titulo}</h2><Badge>{item.cliente_nombre}</Badge><Badge variant={statusVariant(item.estado)}>{item.estado}</Badge></div><p className="mt-2 text-sm text-graphite">{item.descripcion || "Sin descripción adicional."}</p>{item.monto_estimado_usd ? <p className="mt-2 text-sm font-label text-success">Potencial: ${Number(item.monto_estimado_usd).toLocaleString("en-US")} USD</p> : null}</div><div className="flex flex-wrap items-center gap-2"><select value={item.estado} onChange={(event) => onUpdate(`/api/soporte/upsells/${item.id}`, { estado: event.target.value })} className="rounded-component border border-line bg-white px-3 py-2 text-sm"><option value="detectada">Detectada</option><option value="contactada">Contactada</option><option value="propuesta">Propuesta</option><option value="ganada">Ganada</option><option value="perdida">Perdida</option></select>{["detectada", "contactada"].includes(item.estado) ? <Button size="sm" variant="secondary" onClick={() => void createProposal(item)}>Crear propuesta</Button> : null}</div></div></Card>)}</div>;
}
