            ...
            {projectRows.map((project, index) => {
              ...
              return <div key={project.id} className="contents">
                ...
                <div className="relative col-span-16 border-b border-line-soft" style={{ gridColumn: "2 / span 16" }}>
                  ...
                  <div className="pointer-events-none absolute left-0 right-0 top-[44px] h-8">{projectEvents.filter((event) => event.type === "pago").map((event) => <span key={event.id} title={event.label} className="absolute flex h-8 min-w-0 overflow-hidden items-center justify-center gap-0.5 rounded-component border border-emerald-200 bg-emerald-50 px-1 text-[9px] font-label text-emerald-800 shadow-sm" style={{ left: `calc(${event.week * 6.25}% + 2px)`, width: "calc(6.25% - 4px)" }}><DollarSignIcon size={11} className="shrink-0" /><span className="min-w-0 truncate">{event.amount ? formatMoney(event.amount) : "Pago"}</span></span>)}</div>
