            ...
            {projectRows.map((project, index) => {
              ...
              return <div key={project.id} className="contents">
                ...
                <div className="relative col-span-16 border-b border-line-soft" style={{ gridColumn: "2 / span 16" }}>
                  ...
                  <div className="pointer-events-none absolute left-0 right-0 top-[44px] h-8">{projectEvents.filter((event) => event.type === "pago").map((event) => <span key={event.id} title={event.label} className="absolute flex h-8 min-w-[74px] -translate-x-1/2 items-center justify-center gap-1 rounded-component border border-emerald-200 bg-emerald-50 px-2 text-[10px] font-label text-emerald-800 shadow-sm" style={{ left: `${((event.week + 0.5) / 16) * 100}%` }}><DollarSignIcon size={13} /><span>{event.amount ? formatMoney(event.amount) : "Pago"}</span></span>)}</div>
                  <div className="pointer-events-none absolute left-0 right-0 top-[88px] h-8">{projectEvents.filter((event) => event.type === "reunion").map((event) => <span key={event.id} title={event.label} className="absolute flex h-8 min-w-[110px] -translate-x-1/2 items-center justify-center gap-1.5 rounded-component border border-violet-200 bg-violet-50 px-2 text-[10px] font-label text-violet-800 shadow-sm" style={{ left: `${((event.week + 0.5) / 16) * 100}%` }}><VideoIcon size={13} /><span className="max-w-[78px] truncate">{event.date ? formatEventDate(event.date) : event.label}</span></span>)}</div>
                </div>
              </div>;
            })}
          </div>
