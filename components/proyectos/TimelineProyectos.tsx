          ...
          <div className="grid grid-cols-[260px_repeat(48,minmax(52px,1fr))]">
            ...
            {projectRows.map((project, index) => {
              ...
              return <div key={project.id} className="contents">
                <div className="sticky left-0 z-10 grid h-24 grid-cols-[minmax(0,1fr)_42px] grid-rows-[32px_32px_32px] border-b-2 border-line bg-white">
                  ...
                  <div className="row-span-3 grid grid-rows-[32px_32px_32px] border-l border-line text-graphite"><span className="flex items-center justify-center border-b border-line-soft text-xs" title="Duración">■</span><span className="flex items-center justify-center border-b border-line-soft text-sm" title="Hitos de pago">$</span><span className="flex items-center justify-center text-sm" title="Reuniones"><VideoIcon size={15} /></span></div>
                </div>
                <div className="relative h-24 border-l-2 border-b-2 border-line" style={{ gridColumn: `2 / span ${TOTAL_WEEKS}` }}>
                  ...
                  <div className="pointer-events-none absolute left-0 right-0 top-[34px] h-8">{projectEvents.filter((event) => event.type === "pago").map((event) => <span key={event.id} title={event.label} className="absolute flex h-8 min-w-0 overflow-hidden items-center justify-center rounded-component border border-emerald-200 bg-emerald-50 px-1 text-[9px] font-label text-emerald-800 shadow-sm" style={{ left: `calc(${event.week * WEEK_PERCENT}% + 2px)`, width: `calc(${WEEK_PERCENT}% - 4px)` }}><span className="min-w-0 truncate">{event.amount != null ? formatMoney(event.amount) : "$ —"}</span></span>)}</div>
                  <div className="pointer-events-none absolute left-0 right-0 top-[66px] h-8">{projectEvents.filter((event) => event.type === "reunion").map((event) => <span key={event.id} title={event.label} className="absolute flex h-8 min-w-0 overflow-hidden items-center justify-center gap-0.5 rounded-component border border-violet-200 bg-violet-50 px-1 text-[9px] font-label text-violet-800 shadow-sm" style={{ left: `calc(${event.week * WEEK_PERCENT}% + 2px)`, width: `calc(${WEEK_PERCENT}% - 4px)` }}><VideoIcon size={11} className="shrink-0" /><span className="min-w-0 truncate">{event.date ? formatEventDate(event.date) : event.label}</span></span>)}</div>
                </div>
              </div>;
            })}
          </div>
